defmodule Medplum.Resources.RelatedPerson do
  @moduledoc """
  Convenience helpers for FHIR `RelatedPerson` resource.

  ## Examples

      alias Medplum.Resources.RelatedPerson

      {:ok, family} = RelatedPerson.search_by_patient(client, "Patient/patient-123")
      {:ok, active} = RelatedPerson.search_by_active(client, true)
  """
  ...
  @spec search_by_active(Client.t(), boolean(), map()) :: Medplum.result()
  def search_by_active(%Client{} = client, active, params \\ %{}) when is_boolean(active) do
    search(client, Map.merge(params, %{"active" => to_string(active)}))
  end
  ...
  @spec search_by_relationship(Client.t(), String.t(), String.t(), map()) :: Medplum.result()
  def search_by_relationship(%Client{} = client, system, code, params \\ %{}) do
    search(client, Map.merge(params, %{"relationship" => "#{system}|#{code}"}))
  end
end
