defmodule Medplum.Resources.Task do
  @moduledoc """
  Convenience helpers for FHIR `Task` resource.

  ## Examples

      alias Medplum.Resources.Task

      {:ok, tasks} = Task.search_by_owner(client, "Practitioner/practitioner-123")
      {:ok, open_tasks} = Task.search_by_status(client, "requested")
  """
  ...
  @spec search_by_owner(Client.t(), String.t(), map()) :: Medplum.result()
  def search_by_owner(%Client{} = client, owner, params \\ %{}) do
    search(client, Map.merge(params, %{"owner" => owner}))
  end
  ...
  @spec search_by_focus(Client.t(), String.t(), map()) :: Medplum.result()
  def search_by_focus(%Client{} = client, focus, params \\ %{}) do
    search(client, Map.merge(params, %{"focus" => focus}))
  end
end
