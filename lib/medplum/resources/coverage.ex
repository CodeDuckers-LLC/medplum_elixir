defmodule Medplum.Resources.Coverage do
  @moduledoc """
  Convenience helpers for the FHIR `Coverage` resource.

  ## Examples

      alias Medplum.Resources.Coverage

      {:ok, coverages} = Coverage.search_by_beneficiary(client, "Patient/patient-123")
      {:ok, by_id} = Coverage.search_by_identifier(client, "http://payer.example/policy", "ABC-123")
  """
  ...
  @spec search_by_beneficiary(Client.t(), String.t(), map()) :: Medplum.result()
  def search_by_beneficiary(%Client{} = client, beneficiary_id, params \\ %{}) do
    search(client, Map.merge(params, %{"beneficiary" => beneficiary_id}))
  end
