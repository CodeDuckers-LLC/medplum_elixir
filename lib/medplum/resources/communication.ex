defmodule Medplum.Resources.Communication do
  @moduledoc """
  Convenience helpers for FHIR `Communication` resource.

  ## Examples

      alias Medplum.Resources.Communication

      {:ok, messages} = Communication.search_by_patient(client, "Patient/patient-123")
      {:ok, outbound} = Communication.search_by_sender(client, "Practitioner/practitioner-123")
  """
  ...
  @spec search_by_sender(Client.t(), String.t(), map()) :: Medplum.result()
  def search_by_sender(%Client{} = client, sender, params \\ %{}) do
    search(client, Map.merge(params, %{"sender" => sender}))
  end

  @spec search_by_recipient(Client.t(), String.t(), map()) :: Medplum.result()
  def search_by_recipient(%Client{} = client, recipient, params \\ %{}) do
    search(client, Map.merge(params, %{"recipient" => recipient}))
  end
end
