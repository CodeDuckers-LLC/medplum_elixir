defmodule Medplum.Resources.ServiceRequest do
  @moduledoc """
  Convenience helpers for FHIR `ServiceRequest` resource.

  ## Examples

      alias Medplum.Resources.ServiceRequest

      {:ok, orders} = ServiceRequest.search_by_subject(client, "Patient/patient-123")
      {:ok, labs} = ServiceRequest.search_by_code(client, "http://loinc.org", "24323-8")
  """
  ...
  @spec search_by_requester(Client.t(), String.t(), map()) :: Medplum.result()
  def search_by_requester(%Client{} = client, requester, params \\ %{}) do
    search(client, Map.merge(params, %{"requester" => requester}))
  end
end
