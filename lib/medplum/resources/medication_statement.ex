defmodule Medplum.Resources.MedicationStatement do
  @moduledoc """
  Convenience helpers for FHIR `MedicationStatement` resource.

  ## Examples

      alias Medplum.Resources.MedicationStatement

      {:ok, history} = MedicationStatement.search_by_subject(client, "Patient/patient-123")
      {:ok, active} = MedicationStatement.search_by_status(client, "active")
  """
  ...
  @spec search_by_context(Client.t(), String.t(), map()) :: Medplum.result()
  def search_by_context(%Client{} = client, context, params \\ %{}) do
    search(client, Map.merge(params, %{"context" => context}))
  end
end
