defmodule Medplum.Resources.Encounter do
  @moduledoc """
  Convenience helpers for the FHIR `Encounter` resource.

  ## Examples

      alias Medplum.Resources.Encounter

      {:ok, encounters} = Encounter.search_by_patient(client, "Patient/patient-123")
      {:ok, visit} = Encounter.get(client, "encounter-123")
  """
  ...
  def search_by_patient(%Client{} = client, patient_id, params \\ %{}) do
    search(client, Map.merge(params, %{"patient" => patient_id}))
  end

  @spec search_by_appointment(Client.t(), String.t(), map()) :: Medplum.result()
  def search_by_appointment(%Client{} = client, appointment_id, params \\ %{}) do
    search(client, Map.merge(params, %{"appointment" => appointment_id}))
  end
end
