defmodule Medplum.Resources.CarePlan do
  @moduledoc """
  Convenience helpers for FHIR `CarePlan` resource.

  ## Examples

      alias Medplum.Resources.CarePlan

      {:ok, plans} = CarePlan.search_by_subject(client, "Patient/patient-123")
      {:ok, active} = CarePlan.search_by_status(client, "active")
  """
  ...
  @spec search_by_care_team(Client.t(), String.t(), map()) :: Medplum.result()
  def search_by_care_team(%Client{} = client, care_team, params \\ %{}) do
    search(client, Map.merge(params, %{"care-team" => care_team}))
  end
end
